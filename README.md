<div align="center">

# Hey, I'm Niekelle 👋

**Data · AI · Spatial Computing**

*I build things with data. I'm learning to build things with space.*

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/niekellebh)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/niekobloom)
[![Tableau](https://img.shields.io/badge/Tableau-E97627?style=for-the-badge&logo=tableau&logoColor=white)](https://public.tableau.com/app/profile/niekelle.bloomfield/vizzes)



</div>

---

## 🌱 About Me

<img align="right" src="./assets/b8d02272-1177-11ee-8172-3712845f90cf.gif" alt="banner" width="25%"/> <br> <p align="left">
<p>
I'm a Data Analyst and AI/AR developer based in Newark, NJ. I started out analyzing election data at Rutgers, built Tableau dashboards for 70+ stakeholders at Verizon, and now I evaluate AI model outputs to catch where the models go wrong.

I like making data useful and building systems that don't fall apart. I'm also a **Snap Lens Academy Scholar (Summer 2026)** getting into AR development because I think spatial computing is where data storytelling is headed, and I'd rather be building there early. Alongside that, I'm deepening my AI engineering skills through **CodePath's Applications of AI Engineering (Summer 2026)**. 
</p>
<br clear="left" />

---
## 🔭 What I'm Up To


| | |
|---|---|
| 🤖 | **CodePath** - Applications of AI Engineering, Summer 2026 |  
| 👓 | **Snap Lens Academy** - AR Development Scholar, Summer 2026 |
| 🏫 | **The Knowledge House** - Data Analytics Innovation Fellow *(Feb 2026 - Present)* |
| 💼 | **Handshake AI** - AI Analyst, evaluating and validating LLM outputs *(Nov 2025 - Present)* |                                                                                      
| | |


---

## 🛠️ Tech Stack

**Languages & Libraries**

[![My Skills](https://skillicons.dev/icons?i=python,javascript,html,css,numpy&theme=dark)](https://skillicons.dev)

> Python · JavaScript · HTML · CSS · NumPy · Pandas

**Tools & Platforms**

[![My Skills](https://skillicons.dev/icons?i=vscode,git,github,jupyter&theme=dark)](https://skillicons.dev)

> VS Code · Git · GitHub · Jupyter Notebook

**Data & AI**

[![My Skills](https://skillicons.dev/icons?i=postgres&theme=dark)](https://skillicons.dev) ![Tableau](https://img.shields.io/badge/Tableau-E97627?style=for-the-badge&logo=tableau&logoColor=white)

> SQL · Tableau · APIs · AI Engineering · LLM Evaluation · Prompt Engineering

**Spatial Computing & AR**

![Snap Lens Studio](https://img.shields.io/badge/Snap%20Lens%20Studio-FFFC00?style=for-the-badge&logo=snapchat&logoColor=black) [![My Skills](https://skillicons.dev/icons?i=blender&theme=dark)](https://skillicons.dev)

> Snap Lens Studio · AR Development · Spatial UX · Blender

---

## 💼 Experience Highlights

**🤖 AI Analyst @ Handshake** *(Nov 2025 - Present)*
Evaluating 500+ AI-generated outputs to catch accuracy issues, documenting workflows, and working with engineering teams to tighten quality standards.

**📊 Data Science Intern @ Rutgers University** *(Dec 2023 - May 2024)*
Built ETL pipelines in Python to pull and structure election data across all 435 congressional districts. Cut manual processing time by 30%.

**📡 Data Analytics Intern @ Verizon** *(Nov 2023 - Jan 2024)*
Built Tableau dashboards for 70+ stakeholders during a nationwide 5G rollout and put together executive-level analysis in a two-week sprint.

---

## 🎓 Education & Certifications

- 🏛️ **B.A. Computer Science** - Rutgers University *(GPA: 3.4)*
- 🏠 **Innovation Fellow, Data Analytics** - The Knowledge House *(400+ hrs, 2026)*
- 🧠 **Machine Learning Certificate** - Cornell University *(2023)*

---

## 📌 Featured Projects

> *(Coming soon - capstone projects in progress through The Knowledge House, CodePath & Snap Lens Academy)*

---

## 🔬 Currently Exploring

Working at the crossover of data and spatial computing. A few things I'm pulling on right now:

- 📍 **Spatial data visualization** - what does analytics look like in 3D and AR?
- 🧠 **AI-powered AR experiences** - real-time model outputs in the physical world
- 📊 **Immersive dashboards** - data presentation beyond flat screens
- 🔗 **End-to-end AI pipelines** - from raw ingestion to something a person can actually interact with

Open to roles in **Data Analytics, Data Engineering, AI Engineering, and AR/Spatial Computing**, especially where those areas overlap.

---

<div align="center">

*"Turn data into information, and information into insight."*

**Open to roles in Data Analytics, AI Engineering & Spatial Computing - let's connect!**

</div>
